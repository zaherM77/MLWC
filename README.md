...
QF → SF → final) with extra time and Elo-weighted penalty shootouts. Results are
aggregated into per-team probabilities of advancing, reaching each round, and
winning.
...
- **Simplifying assumptions.** Tournament matches are treated as neutral-venue,
  and the Round-of-32 bracket pairings should be confirmed against FIFA's
  official 2026 bracket (the default layout is structurally valid but not
  guaranteed to match official slotting).
...
A minimal, anonymous usage dashboard (visitors, total clicks, clicks per
session) is reachable at `…/?admin=<token>`. There is no login — the token *is*
the secret. Set your own token via the `ADMIN_TOKEN` environment variable, a
`admin_token` entry in `.streamlit/secrets.toml`, or the Streamlit Cloud
**Secrets** UI (otherwise it falls back to `config.DEFAULT_ADMIN_TOKEN`, which
you should change). Counts persist to `data/analytics.json` (best-effort; resets
